...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.c_projection = nn.Linear(2*hidden_size, self.hidden_size, bias=False)
        self.att_projection = nn.Linear(2*hidden_size, self.hidden_size, bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        padded_output = pack_padded_sequence(X, source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(padded_output) 
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for yt in torch.split(Y,1,dim=0):
            ...
            yhat_t = torch.cat([yt,o_prev],dim=1)
            dec_state, combined_output, e_t = self.step(yhat_t,dec_state,enc_hiddens,enc_hiddens_proj,enc_masks)
